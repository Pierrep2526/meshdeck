...
public:
  ...
  // ---- MeshDeck additions ----
  void registerExpectedAck(uint32_t expected_ack, ContactInfo* contact);
  uint32_t sendTracePath(const uint8_t* path, uint8_t path_len);   // returns tag (0 on fail)
  int getNoiseFloorNow() const;
